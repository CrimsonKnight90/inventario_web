...
// Descripción: Tabla especializada para productos con activar/desactivar y editar
...
export default function ProductoTable({ data, onEdit, onActivate, onDeactivate }) {
        ...
        data.map((p) => (
          <tr key={p.id} className={p.activo ? "hover:bg-gray-50" : "bg-gray-100 text-gray-500"}>
            ...
            <td className="px-4 py-2 border space-x-2">
              {/*<AppButton size="sm" variant="secondary" onClick={() => onEdit(p)}>✏️{t("edit", {defaultValue: "Editar"})}</AppButton>*/}
              {p.activo ? (
                <AppButton size="sm" variant="danger" onClick={() => onDeactivate(p.id)}>🗑️{t("producto.deactivated_ok", )}</AppButton>
              ) : (
                <AppButton size="sm" variant="success" onClick={() => onActivate(p.id)}>♻️{t("producto.reactivated_ok", )}</AppButton>
              )}
            </td>
          </tr>
        ))
